...
Passengers::Passengers(float x, float z, Passengers::Priority people, int number, float destX, float destY) : x(x), z(z), people(people), destinationX(destX), destinationZ(destY)
{
	...
	{
	case 0:
		color = GameLogic::Vec3f(0.0f, 0.0f, 0.0f);
		break;
	case 1:
		color = GameLogic::Vec3f(0.0f, 0.0f, 0.0f);
		break;
...
void Passengers::DrawCircle(float cx, float cy, float r, GameLogic::Vec3f color)
{
...
void Passengers::DrawDestination(float x, float z, GameLogic::Vec3f color)
{
	glEnable(GL_COLOR);
	glPointSize(30.0f);
	glBegin(GL_POINTS);
