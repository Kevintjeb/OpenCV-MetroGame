...
int labelIter(Mat & admin, int row, int col, int blobNr, vector<vector<Point2d *>> & coords) {
...
int labelBLOBs(Mat binaryImage, Mat & labeledImage, vector<vector<Point2d *>> &coords) {
	...
	while ((row > 0) & (row < (admin.rows - 1)) &
		...
		if (findNextBlob(admin, row, col))
			labelIter(admin, row, col, ++blobNr, coords);
